...
print(confusion_matrix(output["sentiment"],test["rating"][:36208],normalized=True))
#print(accuracy_score(result,test["rating"]))
# BernoulliNB
print("Bernoulli NB result...")
model = model = BernoulliNB(alpha=0.0005, binarize=0.0, fit_prior=True, class_prior=None)
model=model.fit( train_data_features, train["rating"][:145035] )
print(model)
print("Model is ready..")
result = model.predict(test_data_features)
