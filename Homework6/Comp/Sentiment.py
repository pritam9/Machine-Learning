# -*- coding: utf-8 -*-
"""
Created on Thu Apr 13 14:54:07 2017
...
print(precision_score(test["rating"][:36208],output["sentiment"], average=None))
print(recall_score(test["rating"][:36208],output["sentiment"], average=None))
print("--- %s seconds ---" % (time.time() - start_time))
start_time = time.time()

print("Boosting - AdaBoost Model..")
model = AdaBoostClassifier(DecisionTreeClassifier(max_depth=4),n_estimators=3,learning_rate=0.01)
model=model.fit( train_data_features, train["rating"][:145035] )
result = model.predict(test_data_features)
output = pd.DataFrame( data={"id":test["name"][:36208], "sentiment":result[:36208]} )
...
# GuassinNB
start_time = time.time()
print("Multinomial NB result...")
model = MultinomialNB(alpha=0.0005, fit_prior=True, class_prior=None)
model=model.fit( train_data_features, train["rating"][:145035] )
print(model)
...
output = pd.DataFrame( data={"id":test["name"][:36208], "sentiment":result[:36208]} )
print("Accuracy With Multinomial ..")
print(accuracy_score(output["sentiment"],test["rating"][:36208]))
print(confusion_matrix(output["sentiment"],test["rating"][:36208]))
print(precision_score(test["rating"][:36208],output["sentiment"], average=None))
print(recall_score(test["rating"][:36208],output["sentiment"], average=None))
...
print("NN")
print ("Training NN...")
model = MLPClassifier(solver='adam',alpha=0.001,hidden_layer_sizes=(2),max_iter=200,random_state=1,learning_rate_init=0.01)
model.fit(train_data_features, train["rating"][:145035])
result = model.predict(test_data_features)
...
output = pd.DataFrame( data={"id":test["name"][:36208], "sentiment":prediction[:36208]} )
print("Before printing accuracy")
#print (accuracy_score(prediction, test["rating"][:36208]))
print (accuracy_score(output["sentiment"], test["rating"]))
print(confusion_matrix(output["sentiment"],test["rating"][:36208]))
print(precision_score(test["rating"][:36208],output["sentiment"], average=None))
print(recall_score(test["rating"][:36208],output["sentiment"], average=None))
print("--- %s seconds ---" % (time.time() - start_time))